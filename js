...
// Stores users in Upstash Redis with hashed passwords. Role-based.
// Actions: login, changePassword, list, add, remove, setRole, seedOwner
...
module.exports = async function handler(req, res) {
  try {
    ...
    // First-run: if no users exist, allow seeding the owner account
    if (action === 'seedOwner') {
    ...
    // --- Admin actions below require the caller to be owner/manager ---
    // We trust the role passed because login is server-verified and session-held client-side.
    // For stronger security later, swap to signed tokens.
    ...
    // Check if any users exist (for showing setup screen)
    if (action === 'status') {
